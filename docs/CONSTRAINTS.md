...
- **MEMAC window must not overlap the soft stack.** VRAM is reached
  from the CPU through a banked MEMAC aperture (default MEMAC-A at
  `$B000-$BFFF`). While the window is enabled, *every* CPU access in
  that range goes to VRAM instead of RAM — including the llvm-mos soft
  (call) stack, which the atari8-dos runtime places at the top of RAM
  (observed ~`$BC01`, inside the default window). An overlapping window
  aliases the call stack onto VRAM on every VRAM access, corrupting both
  the display and the program (progressive noise, then a crash). Place
  the window in free RAM clear of the stack and heap (e.g.
  `MEMAC_A_Cfg<0xA0>` → `$A000-$AFFF`); diagnose via the soft-stack
  pointer at ZP `$80`/`$81`. (See ADR-030; `arena_vbxe` is the example.)
