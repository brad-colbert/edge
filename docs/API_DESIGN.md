...
**VBXE overlay screen** (`gfx::VBXE<...>` platforms): a screen can
declare an `OverlayRegion` instead of (or alongside) ANTIC
regions. Its pixels live in VBXE VRAM, so it costs no screen
buffer:

```cpp
struct OverlayScreen {
    using display = engine::DisplayLayout<
        engine::OverlayRegion<atari::Mode::VBXE_SR, 240>
    >;
};
```
...
over the ANTIC content (region order sets its vertical position).
...
4. Registers a raw raster hook at each zone boundary that writes
   the new zone's HPOSP0-3 *and* COLPM0-3 from a flat pre-baked
   table (~150 cycles per hook). Colours are switched per zone
   here because the DLI runs after the OS PCOLR→COLPM copy; zone 0
   (set in the VBI, before that copy) goes through the PCOLR
   shadow so it isn't overwritten. The boundary scanline is biased
   up by one mode line so the switch lands in the inter-zone gap.
5. The game's render phase pre-writes all sprite shape data
   into P/M memory at the correct Y offsets — the hook only
   changes horizontal positions and colours, not shape data
...
Scroll is a per-region capability: wrap any `TextRegion` / `BitmapRegion` in
`ScrollRegion<Inner, MapWidth, MapHeight>` inside the screen's `DisplayLayout`.
The wrapper keeps the inner region's mode, view, and visible height, and adds
the map geometry. It is mode-agnostic — the same machinery scrolls text or
bitmap modes.

```cpp
struct PlayScreen {
    using display = engine::DisplayLayout<
        engine::TextRegion<atari::Mode::MODE_2, 2>,                       // fixed HUD
        engine::ScrollRegion<engine::TextRegion<atari::Mode::MODE_2, 22>, // 22 visible rows
                             64, 32>>;                                    // 64×32 map
};
```

### Binding a map and moving

The scroll region's pixels live in a game-held `engine::TileMap` (it costs no
shared screen RAM — the map *is* the screen memory ANTIC reads). Bind it after
`init()`, then drive the viewport:

```cpp
static engine::TileMap<64, 32> map;       // fill at startup
Game::scroll_map(map);                    // bind as the scroll source + activate

Game::scroll.set(x, y);                   // absolute viewport position
Game::scroll.move(dx, dy);                // relative
```

Each frame the engine splits the position into fine scroll (the sub-cell
remainder, written to the fine-scroll registers) and coarse scroll (whole cells,
one load address per visible line patched into the display program), clamps at
the map edges, and keeps `Game::tiles` viewport in sync. Horizontal `x` is in
fine units, vertical `y` in scanlines.

### Map margins (horizontal)

Enabling horizontal scroll makes the display hardware fetch a *wider* line than
it shows, so the visible window sits a few columns into the fetched data. Reserve
those columns as left margin in the map (e.g. ~4 columns for Mode 2) so logical
column 0 lands at the visible edge rather than in the border. Coarse scrolling is
clamped to the fetch width so it never reads past the map's right edge.

### Split-screen scroll

For a scrolling playfield with a fixed status bar, put a plain `TextRegion` and a
`ScrollRegion` in the same `DisplayLayout`. The display hardware renders regions
top to bottom, so a fixed bar above (or below) a scrolling field needs no raster
hook — only the scroll region carries the fine-scroll enable.

### Scroll suspension

```cpp
Game::scroll.suspend();   // engine stops writing the scroll hardware
