...
//   - pre_sprite_commit: runs inside the frame service, immediately before the
//     sprite manager commits buffered positions to sprite memory. Use it to
...
// (DECISIONS.md ADR-020), exactly as raster/frame handlers do. This header
// depends on nothing.

namespace engine {

struct Hooks {
    void (*pre_sprite_commit)() = nullptr;   // frame service, before sprite commit
